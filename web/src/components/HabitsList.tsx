...
export function HabitsList({ date, onCompletedChanged }: HabitsListProps) {
    ...
    async function handleToggleHabit(habitId: string) {
        ...
        // javascript pode reconhecer que essa informação não irá existir, mas ela vai, por isso o '!' em 'habitsInfo!'

        const isHabidAlreadyCompleted = habitsInfo!.completedHabits.includes(habitId)

        let completedHabits: string[] = []

        if (isHabidAlreadyCompleted) {
            // remover
            ...
            <div className='mt-6 flex flex-col gap-3'>
                {habitsInfo?.possibleHabits.map(habit => {
                    return (
                        ...
                        >
                            {/* utilizando a classe 'group', é possível fazer estilizações baseadas em propriedades que um elemento pai possui  */}
